...
class Crawler:
    ...
    def fetch_data_from_url(url):
        ...
        if response.status_code == 200:
            ...
            cooking_time = soup.find('span' , class_ = 'wprm-recipe-time wprm-block-text-bold')
    ...
    def crawl():    
        for page in range(2,Crawler.pageCount):
            url = "https://www.skinnytaste.com/recipes/page/" + str(page)
            print(page)
            response = requests.get(url=url,headers=Crawler.headers)
            # Results = []
            soup = BeautifulSoup(response.text , 'html.parser')
            resultsRecipes = soup.find_all('h3' , class_ = 'entry-title ast-blog-single-element')
